  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50/30 to-stone-100 text-stone-900 font-sans flex flex-col justify-between">
      ...
      <main className="max-w-5xl mx-auto px-6 py-20 flex-1">
        <span className="text-xs font-bold text-amber-600 uppercase tracking-wider bg-amber-100 px-3 py-1.5 rounded-full">Careers</span>
        
        <div className="grid lg:grid-cols-2 gap-12 mt-6 items-center">
          <div className="space-y-6">
            <h2 className="text-4xl font-black mt-4">Join the BrewOps Team</h2>
            <p className="text-stone-600 leading-relaxed">
              We are always looking for passionate engineers, designers, and F&B experts to build the future of cafe operations. View our current openings or send your CV directly to <a href="mailto:careers@brewops.com" className="text-amber-600 hover:underline">careers@brewops.com</a>.
            </p>
            
            <div className="space-y-4 pt-2">
              <h3 className="font-extrabold text-sm uppercase tracking-wide text-stone-400">Current Openings</h3>
              {positions.map((pos, idx) => (
                <div key={idx} className="p-4 bg-white border border-stone-200 rounded-2xl flex items-center justify-between hover:border-amber-500 transition-all cursor-pointer">
                  <div>
                    <h4 className="font-bold text-xs">{pos.title}</h4>
                    <span className="text-[10px] text-stone-500">{pos.department} • {pos.location}</span>
                  </div>
